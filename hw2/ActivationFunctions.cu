...
Tensor ReLU::backward(const Tensor& grad_output, const Tensor& input) {
    if (grad_output.shape() != input.shape()) {
        throw std::invalid_argument("Gradient output and input shapes must match");
    }
...
Tensor Sigmoid::backward(const Tensor& grad_output, const Tensor& output) {
    if (grad_output.shape() != output.shape()) {
        throw std::invalid_argument("Gradient output and function output shapes must match");
    }
