...
def parse_mnist():
    ...
    X_tr, y_tr = dataset_to_numpy(train_dataset)
    X_te, y_te = dataset_to_numpy(test_dataset)
...
def softmax_loss(Z, y):
    ...
    return -np.mean(log_probs[np.arange(y.size), y])
...
def SGD_epoch(X, y, weights, lr = 0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        hidden = np.maximum(0, X_batch @ W1 + b1)
        logits = hidden @ W2 + b2
        probs = np.exp(logits - logits.max(axis=1, keepdims=True))
        probs /= probs.sum(axis=1, keepdims=True)
        probs[np.arange(y_batch.shape[0]), y_batch] -= 1
        probs /= y_batch.shape[0]

        grad_W2 = hidden.T @ probs
        grad_b2 = probs.sum(axis=0)
        hidden_grad = (probs @ W2.T) * (hidden > 0)
        grad_W1 = X_batch.T @ hidden_grad
...
def Adam_epoch(X, y, weights, lr = 0.1, batch=100, beta1=0.9, beta2=0.999):
    ...
    Returns:
        None
    """
    if not hasattr(Adam_epoch, "_state"):
        Adam_epoch._state = {}
    state = Adam_epoch._state.setdefault(id(weights), {
    ...
    for start in range(0, num_examples, batch):
        ...
        # state["t"] += 1
        for idx, (param, grad) in enumerate(zip(weights, grads)):
            state["m"][idx] = beta1 * state["m"][idx] + (1 - beta1) * grad
            state["v"][idx] = beta2 * state["v"][idx] + (1 - beta2) * (grad * grad)
            m_hat = state["m"][idx] / (1 - beta1 ** state["t"])
            v_hat = state["v"][idx] / (1 - beta2 ** state["t"])
...
if __name__ == "__main__":
    ...
    # using SGD optimizer 
    train_nn(X_tr, y_tr, X_te, y_te, weights, hidden_dim=100, epochs=20, lr = 0.1, batch=256, beta1=0.9, beta2=0.999, using_adam=False)
    ## using Adam optimizer
    # train_nn(X_tr, y_tr, X_te, y_te, weights, hidden_dim=100, epochs=20, lr = 0.005, batch=256, beta1=0.9, beta2=0.999, using_adam=True)
